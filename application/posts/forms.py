...
class PostForm(FlaskForm):
  ...
  text = StringField("text", widget=TextArea(), validators=[Optional(), Length(max=3000)])
...
class EditTextPostForm(FlaskForm):
  title = StringField("title", validators=[Length(min=2, max=255)])
  content = StringField("text", widget=TextArea(), validators=[Length(max=3000)])

  class Meta:
    csrf = False

class CommentForm(FlaskForm):
  content = StringField("Add comment", widget=TextArea(), validators=[Length(min=2, max=3000)])
